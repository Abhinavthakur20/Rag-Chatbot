...
async function buildIndex() {
  const { version } = getVersionedIndexPath();
  // determine rootPath and indexPath, but handle environments where process.cwd()
  // is read-only (e.g., serverless). Try to use .vectra in project root, else
  // fall back to OS temp directory.
  let rootPath = path.join(process.cwd(), ".vectra");
  let indexPath = path.join(rootPath, `prompt-kb-${version}`);
  const manifestPath = path.join(indexPath, "manifest.json");
  const LocalIndex = await getLocalIndex();

  // attempt to create directories; if that fails due to permissions or read-only
  // filesystem, switch to a tmp directory which is usually writable in serverless
  try {
    await fs.mkdir(rootPath, { recursive: true });
    await fs.mkdir(indexPath, { recursive: true });
  } catch (err) {
    try {
      const tmpRoot = path.join(os.tmpdir(), ".vectra");
      rootPath = tmpRoot;
      indexPath = path.join(rootPath, `prompt-kb-${version}`);
      await fs.mkdir(rootPath, { recursive: true });
      await fs.mkdir(indexPath, { recursive: true });
    } catch (err2) {
      // If even the tmp dir is not writable, rethrow the original error so the
      // app surfaces a clear message.
      throw new Error(
        "Unable to create vectra index directory in project or tmp directory: " +
