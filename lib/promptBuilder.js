...
export function normalizeSelections(selections = {}) {
  ...
  return {
    ...
    tool,
    toolRule: tool ? toolRuleMap[tool] || null : null,
...
export function summarizeSelections(selections = {}) {
  ...
  const { characterDetails, toolRule } = normalized;
  const detailParts = [
...
export function formatSelectionLines(selections = {}) {
  const summary = summarizeSelections(selections);
  return [
...
export function buildGenerationPrompt({
  ...
  const selectionLines = formatSelectionLines(selections);
